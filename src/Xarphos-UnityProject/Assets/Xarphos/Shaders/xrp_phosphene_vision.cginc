    ...
    // Additional pre-processing filters
    float xrp_dilate(sampler2D edge_mask, float2 position, float2 resolution)
    {
        // Sample edge_mask at [position] and surrounding pixels
        #define SAMP(offset) Luminance(tex2D(edge_mask, position + offset))
        float diag = 0.70710678118; // factor sqrt(2)/2, for the diagonal samples 
        float pixel_spacing=1/resolution.x;
        ...
        //float3x3 a = float3x3(SAMP(float2(-1, 1)), SAMP(float2(0, 1)), SAMP(float2(1, 1)), SAMP(float2(-1, 0)), SAMP(float2(0, 0)), SAMP(float2(1, 0)), SAMP(diag * float2(-1, -1)), SAMP(float2(0, -1)), SAMP(diag * float2(1, -1)));
